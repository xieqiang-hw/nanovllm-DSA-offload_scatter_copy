...
function build_package(){
    prepare_generated_opc_soc_version
    if [ -n "${OPS_OVERLAP_OPC_SOC_VERSION:-}" ]; then
        build ops_kernel
    fi
    build package
    patch_generated_opc_soc_version
}
...
function patch_generated_opc_soc_version()
{
    local opc_soc_version="${OPS_OVERLAP_OPC_SOC_VERSION:-}"
    if [ -z "${opc_soc_version}" ]; then
        return
    fi

    local generated_dir="${BUILD_DIR}/binary"
    if [ ! -d "${generated_dir}" ]; then
        log "Warning: OPS_OVERLAP_OPC_SOC_VERSION=${opc_soc_version}, but ${generated_dir} does not exist."
        return
    fi

    local patched_count=0
    while IFS= read -r script_path; do
        if grep -q -- "--soc_version=" "${script_path}"; then
            sed -i -E "s/--soc_version=[^[:space:]]+/--soc_version=${opc_soc_version}/g" "${script_path}"
            patched_count=$((patched_count + 1))
        fi
    done < <(find "${generated_dir}" -path "*/gen/*.sh" -type f 2>/dev/null)

    if [ "${patched_count}" -eq 0 ]; then
        log "Warning: OPS_OVERLAP_OPC_SOC_VERSION=${opc_soc_version}, but no generated opc scripts were patched."
    else
        log "Info: patched ${patched_count} generated opc scripts to --soc_version=${opc_soc_version}."
    fi
}

function prepare_generated_opc_soc_version()
{
    if [ -z "${OPS_OVERLAP_OPC_SOC_VERSION:-}" ]; then
        return
    fi
...
if [ "${BUILD}" == "host" ];then
    ...
    cmake_config
    prepare_generated_opc_soc_version
    if [ "${BUILD}" == "package" ] && [ -n "${OPS_OVERLAP_OPC_SOC_VERSION:-}" ]; then
        build ops_kernel
    fi
    build ${BUILD}
    patch_generated_opc_soc_version
