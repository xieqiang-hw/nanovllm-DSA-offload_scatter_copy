...
三个分支使用相同的入口、参数和六列结果。本分支测试 **A5 BF16**；每个有效 token 为 **1152 字节**，默认 `COPY_CAP=2048`。BF16 需要 HBM slots 大于实际 copy count，以保留 guard token。
...
默认输出目录为 `results/scatter_copy_a5`，各分支分开保存，防止切换分支后混入其他格式。每组生成 JSON 和日志，并生成统一命名的汇总文件：
...
python3 tests/analyze_scatter_copy_results.py \
  --results-dir results/scatter_copy_a5
python3 tests/format_scatter_copy_csv.py \
  --input results/scatter_copy_a5/scatter_copy_timing_summary.csv
```
