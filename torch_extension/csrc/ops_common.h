...
// to this standalone operator extension.
...
{
    append_custom_opapi_lib_dir(path_list, opp_path + "/vendors/ops-overlap");
}
...
inline std::vector<std::string> get_custom_lib_path()
{
    ...
    char *overlap_install_opp_path = std::getenv("OPS_OVERLAP_INSTALL_OPP_PATH");
    if (overlap_install_opp_path != nullptr) {
        append_custom_vendor_from_opp(custom_lib_path_list, overlap_install_opp_path);
    }
